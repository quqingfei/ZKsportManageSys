...
function saveArticle() {
	if(checkForm()){
		if(addoredit=="add"){
			$.ajax({
				type: 'post',
				url: './AssociationAction!addMatch.zk',
				data: consie,
				dataType:'json',
			...
			$.ajax({
				type: 'post',
				url: './AssociationAction!updateMatch.zk',
				data: consie,
				dataType:'json',
				success: function(res){
					if(res.STATUS){
...
function del() {
	...
	if(row){
		if (confirm('确定删除该版本吗?')) {
			$.getJSON("./AssociationAction!deleteMatch.zk", {
				id: row.id
			}, function(data) {
				if (data.STATUS) {
...
/*$('#into').change(function(){
	console.log($('#into').val());
	$.ajax({
		type:'get',
		url: './AssociationAction!updateMatchOrderStateForExcel.zk',
...
function outtosubmit(){
	...
	window.open("http://"+window.location.host+"/fatburn/manage/AssociationAction!writeMatachUserToExcel.zk?checkStatus="+s); 
	$('#dlgame').dialog('close')
}
function searchBox(){
...
KindEditor.ready(function(K) {
    ...
    editor = K.create('#editor_id', {
        ...
        items: [
            ...
            'insertunorderedlist', '|', 'link', 'source', 'example1'
        ],
    });
...
function showuser(){
	...
	if(row){
		mid = row.id;
   		$('#dguser').datagrid({
   			url : 'AssociationAction!getMatchUser.zk',
   			queryParams : { matchId:row.id }
   		});
...
function searchUser(){
    ...
    $("#dguser").datagrid('load',{checkStatus:searchState,name:name,phone:phoneNumber,matchId:mid});
}
...
function status(e){
	$.ajax({
		type:'post',
		dataType:"json",
		url:'/fatburn/manage/AssociationAction!updateStateForMatchOrder.zk',
		data:{matchOrderId:$(e).attr('gs'),checkStatus:$(e).val()},
		success: function(res){
			showTip(res.INFO);
		}
	})
